...
public class PowerUp : MonoBehaviour
{
    ...
    void Awake()
    {
        // Find the Cube reference (there's one a single child) 
        ...
        vel.Normalize(); // Normalizing a Vector3 sets its lenght to lm
        ...
        // Randomize rotPerSecond for PowerCube using rotMinMax x and y
    ...
    void Update()
    {
        ...
        // Given the default values, a PowerUp will exist for 10 seconds and then fade otu over 4 seconds
        ...
        // If u>0, decrease the opacity (i.e., alpha) of the PowerCube and Letter
        if(u>0) {
            ...
            c = letter.color;
            c.a = 1f - (u*0.5f); // set the alpha of the leter to 1-(u/2)
            letter.color = c;
        }

        if (!bndCheck.isOnScreen) {
            // If the PowerUp has drifter entirely off screen, destroy it
            Destroy( gameObject );
        }
    }
    ...
    public void SetType( eWeaponType wt ) {
        ...
        type = wt; // Finallyactually set the type
    }
